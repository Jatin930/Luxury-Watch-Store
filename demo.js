...
app.get ("/cartier", function (req,res)
{
 if(req.session.flag =="1")
 res.render ( "cartier.ejs",{message:"",flag:"1"});
 else
 {
...
app.get ("/rolex", function (req,res)
{
 if(req.session.flag =="1")
 res.render ( "rolex.ejs",{message:"",flag:"1"});
 else
 {
 req.flash('message', 'ERROR: Must Login First');
 res.redirect ( "login");
...
{
 if(req.session.flag =="1")
 res.render ( "versace.ejs",{message:"",flag:"1"});
 else
 {
 req.flash('message', 'ERROR: Must Login First');
 res.redirect ( "login");
        ...
        {       
            ...
            //{
                ...
                { 
                    ...
                    req.flash('message', 'Account has been made');
                    res.redirect ( "login");
                });
            //});
        }
...
app.post("/add", (req, res) => 
{
    ...
    Cart.find(item2find, '', function (err, data) 
    {
		...
		if(data=="")
		{
			...
			{
				if (err) return handleError(err);
				var message="Iteim Added to Cart";
				console.log(message);
			});
		}
		else
...
app.get("/cart", (req, res) => 
{
    ...
    Cart.find(item2find, '', function (err, data) 
    {
		...
		if(data=="")
		{
			var cart="CART EMPTY<br><br><a href='/products'>Back To Shopping</a>";
		...
		else
		{
			...
			{
				var image="images/"+data[i].code+".png";
			    cart+="<tr><td><img style=' width:150px' src='"+image+"' /></td><td>"+data[i].name+"</td><td><div style='vertical-align: middle;'><form style='float:left' method=post action='/change'><input style='float:left' type=hidden name=code value='"
			...
			else cart+="<br><div 'class=cart' style='width:100%;padding-left:530px'><a href='/checkout'>Checkout</a></div>"
			res.render ( "cart.ejs",{cart:cart,flag:req.session.flag,message:""});
		}
